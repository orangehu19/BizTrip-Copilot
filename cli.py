...
class AligoCLI:
    ...
    def print_banner(self):
        """打印欢迎横幅"""
        self.console.print("\n[bold cyan]🌏 BizTrip Copilot 商旅助手[/bold cyan] - 让差旅更简单\n", style="bold")
    ...
    async def initialize_system(self):
        ...
        with self.console.status("初始化中...", spinner="dots"):
            ...
            if not LLM_CONFIG.get("api_key"):
                self.console.print(
                    "\n[bold red]缺少大模型 API Key。[/bold red]\n"
                    "请在项目根目录创建 `.env`，或在当前 PowerShell 会话设置环境变量：\n\n"
                    "[cyan]$env:BIZTRIP_API_KEY=\"你的 API Key\"[/cyan]\n"
                    "[cyan]$env:BIZTRIP_MODEL_NAME=\"deepseek-v4-pro\"[/cyan]\n"
                    "[cyan]$env:BIZTRIP_BASE_URL=\"https://api.deepseek.com\"[/cyan]\n",
                    style="white",
                )
                raise SystemExit(1)
